...
- [x] 管理后台路由 & 权限守卫（role = ADMIN）
- [x] 用户管理页面（列表、搜索、禁用/启用、详情）
- [x] 用量概览仪表盘（Token / 图片 / 积分消耗图表）
- [x] 计费设置页面（套餐管理、额度配置、定价规则）
- [x] 模型配置页面（启用/禁用模型、费率设置、API Key 管理）
- [x] 系统设置页面（注册开关、默认额度、文件大小限制）
- [x] 操作日志 & 审计记录
...
### PPTX 导出（混合策略）
- [x] 结构化元素转换（文本 → TextBox，图片 → Image，表格 → Table）
